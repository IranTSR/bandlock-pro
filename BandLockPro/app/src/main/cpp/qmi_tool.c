...
static int cmd_band_lock(const char *lte_str, const char *nr_str) {
    ...
    uint64_t nr_mask = nr_str ? strtoull(nr_str, NULL, 0) : 0;
    ...
    /* TLV 0x2C: NR5G band preference (if specified) */
    if (nr_mask) {
        put_u8(tlv, &pos, TLV_NR5G_BAND_PREF);
        put_le16(tlv, &pos, 8);
        put_le64(tlv, &pos, nr_mask);
    }
    ...
    if (rc == 0) {
        ...
        if (nr_mask)
            printf(",\"nr_bands\":\"0x%llx\"", (unsigned long long)nr_mask);
...
static void usage(void) {
    printf(
        ...
        "  band_lock <lte> [nr]    Lock to specific bands (hex bitmask)\n"
        ...
        "  band_lock 0x4 0x2000000000000000000  B3 + NR n78\n\n"
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "band_lock") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: qmi_tool band_lock <lte_mask> [nr_mask]\n"); return 1; }
