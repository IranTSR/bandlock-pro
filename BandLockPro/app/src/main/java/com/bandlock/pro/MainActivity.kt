            ...
            if (result.isSuccess) {
                try {
                    ...
                    if (lteBandsHex.isNotEmpty()) {
                        ...
                        if (lteMask == ALL_BANDS_MASK || lteMask == 0xFFFFFFFFFFFFFFFFUL) {
                            runOnUiThread {
                                binding.tvLockState.text = "🔓 UNLOCKED (All Bands)"
                            ...
                            for (b in 1..64) {
                                if (lteMask and (1UL shl (b - 1)) != 0UL) {
                                    bands.add(bandMap[b] ?: "B$b")
                                }
                            }
                            val label = bands.joinToString(" + ")
                            runOnUiThread {
                                binding.tvLockState.text = "🔒 LOCKED: $label"
                                binding.tvLockState.setTextColor(0xFFFBBF24.toInt())
                            }
                        }
                    }
                } catch (e: Exception) {
